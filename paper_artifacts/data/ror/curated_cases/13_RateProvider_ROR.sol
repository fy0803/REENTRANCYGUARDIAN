...
interface IUnwrapObserver13 {
    function onUnwrapStaged13(address owner, uint256 assets) external;
}

contract RORRateProviderToken13 {
    RORRateLedger13 public immutable ledger;

    constructor(RORRateLedger13 ledger_) {
        ledger = ledger_;
    }
    ...
    function unwrap(uint256 shareAmount, IUnwrapObserver13 observer) external {
        uint256 assets = ledger.stageUnwrap(msg.sender, shareAmount);
        observer.onUnwrapStaged13(msg.sender, assets);
        ledger.commitUnwrap(msg.sender, shareAmount, assets);
    }
}

contract RORRateLedger13 {
...
contract RORRateBasedVault13 {
    RORRateProviderToken13 public immutable token;
    mapping(address => uint256) public credit;

    constructor(RORRateProviderToken13 token_) {
        token = token_;
    }
